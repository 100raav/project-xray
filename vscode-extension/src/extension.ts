...
interface GalaxyNode {
  id:string; kind:string; name:string; qualifiedName:string; file:string; line:number;
  packageName:string; frameworkRole?:string|null; inboundRelations:number; outboundRelations:number;
}
interface GalaxyEdge { sourceId:string; targetId:string; kind:string; evidenceCount:number; }
interface ImpactPath {
  targetId:string; direction:string; distance:number; path:string[];
  relationKinds:string[]; evidenceFiles:string[]; evidenceLines:number[];
}
interface ChangeImpact {
  sourceId:string; sourceName:string; directDependents:number; transitiveDependents:number;
  directDependencies:number; transitiveDependencies:number;
  affectedCallers:ImpactPath[]; affectedDependencies:ImpactPath[];
  affectedEndpoints:string[]; affectedTests:string[]; warnings:string[];
}
interface GitTimeMachine {
  repository:boolean;
  commits:Array<{hash:string;shortHash:string;author:string;authoredAt:string;subject:string;changedFiles:number}>;
  snapshots:Array<{commitHash:string;subject:string;analyzedAt:string;filesScanned:number;entities:number;relations:number;endpoints:number;packages:number;cycles:number;warnings:string[]}>;
  currentDiff:{fromCommit:string;toCommit:string;addedEntities:string[];removedEntities:string[];changedFiles:string[];addedPackages:string[];removedPackages:string[];relationDelta:number;endpointDelta:number;cycleDelta:number};
  warnings:string[];
}
interface AnalysisResult {
  schemaVersion:string; analyzedAt:string; project:string; rootPath:string; language:string;
  durationMs:number; filesScanned:number;
  entities:Array<{id:string;kind:string;name:string;qualifiedName:string;file:string;line:number;language:string;frameworkRole?:string|null}>;
  relations:Array<{sourceId:string;targetId:string;kind:string;evidenceFile?:string|null;evidenceLine?:number;evidence?:string|null}>;
  endpoints:Array<{id:string;httpMethod:string;route:string;controllerId:string;methodId:string;file:string;line:number}>;
  warnings:string[];
  build:{system:string;group:string;artifact:string;version:string;modules:string[];dependencies:string[];warnings:string[]};
  git:{repository:boolean;branch:string;commit:string;dirty:boolean;changedFiles:string[];remote:string};
  files:Array<{path:string;size:number;modifiedEpochMs:number;sha256:string}>;
  architectureGraph:{rootId:string;nodes:any[];edges:any[];cycles:string[][];maxDepth:number};
  dependencyGalaxy:{rootId:string;nodes:GalaxyNode[];edges:GalaxyEdge[];cycles:string[][];maxDepth:number};
  gitTimeMachine:GitTimeMachine;
  scanDiagnostics:{complete:boolean;strategy:string;javaFilesDiscovered:number;javaFilesAnalyzed:number;sourceBytesAnalyzed:number;limitation:string};
  codeHealthRadar:{score:number;metrics:Array<{id:string;name:string;category:string;value:number;unit:string;status:string;explanation:string;evidence:string}>;findings:Array<{severity:string;category:string;title:string;entityId:string;file:string;line:number;explanation:string;evidence:string}>;warnings:string[]};
}

class XRayProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  private readonly emitter=new vscode.EventEmitter<void>();
  readonly onDidChangeTreeData=this.emitter.event;
  private root:vscode.TreeItem[]=[];
  refresh(items:vscode.TreeItem[]){this.root=items;this.emitter.fire();}
...
class XRayState {
  ...
  set(a:AnalysisResult){this.analysis=a;this.emitter.fire();}
  get(){return this.analysis;}
}
class XRayTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem>{
  constructor(private readonly state:XRayState){}
  private readonly emitter=new vscode.EventEmitter<void>();
  readonly onDidChangeTreeData=this.emitter.event;
  ...
  getChildren():vscode.TreeItem[]{
    ...
    if(!a)return [new vscode.TreeItem('Run Analyze Project')];
...
function item(label:string,command:string){
...
function coreJarPath(context:vscode.ExtensionContext):string {
  const configured=vscode.workspace.getConfiguration('projectXray').get<string>('coreJarPath');
  if(configured)return configured;
  return path.resolve(context.extensionPath,'..','core','target','xray-core-1.0.0.jar');
}

async function runAnalysis(context:vscode.ExtensionContext,folder:vscode.WorkspaceFolder):Promise<AnalysisResult>{
  const jar=coreJarPath(context);
  if(!fs.existsSync(jar))throw new Error(`Project X-Ray core JAR not found at ${jar}. Build the core with Maven first or configure projectXray.coreJarPath.`);
  ...
  return await new Promise((resolve,reject)=>{
    ...
    let stderr='';child.stderr.on('data',(d:Buffer)=>stderr+=d.toString());child.on('error',reject);
    child.on('close',(code:number|null)=>{
      if(code!==0){reject(new Error(stderr.trim()||`X-Ray core exited with code ${code}`));return;}
      try{resolve(JSON.parse(fs.readFileSync(output,'utf8')) as AnalysisResult);}
      catch(e){reject(new Error(`Analysis completed but result could not be read: ${String(e)}`));}
    });
  });
}

async function runImpact(context:vscode.ExtensionContext, root:string, entityId:string):Promise<ChangeImpact>{
  const jar=coreJarPath(context);
  if(!fs.existsSync(jar))throw new Error(`Project X-Ray core JAR not found at ${jar}. Build the core with Maven first or configure projectXray.coreJarPath.`);
  return await new Promise((resolve,reject)=>{
    ...
    child.on('close',(code:number|null)=>{
      if(code!==0){reject(new Error(stderr.trim()||`Impact analysis exited with code ${code}`));return;}
      try{resolve(JSON.parse(stdout) as ChangeImpact)}
      catch(e){reject(new Error(`Impact analysis completed but its result could not be read: ${String(e)}`));}
    });
  });
}

function openVisualization(context:vscode.ExtensionContext,data:AnalysisResult){
  const panel=vscode.window.createWebviewPanel(
    'projectXray.galaxy',`X-Ray · ${data.project}`,vscode.ViewColumn.One,
  ...
  panel.webview.onDidReceiveMessage(async(message:any)=>{
    ...
    if(message.type==='openFile'){
      ...
      }catch(e){vscode.window.showErrorMessage(`Project X-Ray: cannot open ${message.file}: ${String(e)}`);}
    }
  });
}

function getGalaxyHtml(serialized:string):string{
...
export function activate(context:vscode.ExtensionContext){
  ...
  const status=vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right,100);status.text='$(pulse) X-Ray: idle';status.show();context.subscriptions.push(status);
  let timer:ReturnType<typeof setTimeout>|undefined;
  const analyze=vscode.commands.registerCommand('projectXray.analyzeProject',async()=>{
    const folder=vscode.workspace.workspaceFolders?.[0];
    if(!folder){vscode.window.showErrorMessage('Project X-Ray: open a project folder first.');return}
  ...
  const openCompilerContext=vscode.commands.registerCommand('projectXray.openCompilerContext',async()=>{
    const folder=vscode.workspace.workspaceFolders?.[0];
    if(!folder){vscode.window.showErrorMessage('Project X-Ray: open a workspace folder first.');return}
    const uri=vscode.Uri.file(vscode.Uri.joinPath(folder.uri,'.xray','compiler-context.json').fsPath);
    try{const doc=await vscode.workspace.openTextDocument(uri);await vscode.window.showTextDocument(doc,{preview:true});}
    catch{vscode.window.showInformationMessage('Project X-Ray: compiler context not generated yet. Run Analyze Project first.');}
  });
    ...
    try{
      ...
      let matches=symbols.filter((x:any)=>x.id===query);
      let strategy='symbol-id';
      if(matches.length===0){matches=symbols.filter((x:any)=>x.qualifiedName===query);strategy='qualified-name';}
      if(matches.length===0){matches=symbols.filter((x:any)=>x.name===query);strategy='simple-name';}
      ...
      const doc=await vscode.workspace.openTextDocument(uri);await vscode.window.showTextDocument(doc,{preview:true});
    }catch{vscode.window.showInformationMessage('Project X-Ray: no persistent symbol store yet. Run Analyze Project first.');}
  });
    ...
    try{const doc=await vscode.workspace.openTextDocument(uri);await vscode.window.showTextDocument(doc,{preview:true});}
    catch{vscode.window.showInformationMessage('Project X-Ray: no invalidation plan yet. Run Analyze Project first.');}
  });
  ...
  const openReport=vscode.commands.registerCommand('projectXray.openReport',async()=>{
    const f=vscode.workspace.workspaceFolders?.[0];if(!f)return;
    const report=vscode.Uri.file(path.join(f.uri.fsPath,'.xray','analysis.json'));
    try{await vscode.window.showTextDocument(report)}catch{vscode.window.showWarningMessage('Project X-Ray: no analysis exists yet. Run Analyze Project first.')}
  });
  const openTimeMachine=vscode.commands.registerCommand('projectXray.openTimeMachine',async()=>{
    ...
    try{
      const d=await runAnalysis(context,folder); state.set(d); provider.refresh();
  ...
  const watcher=vscode.workspace.onDidSaveTextDocument((doc:vscode.TextDocument)=>{
    if(!vscode.workspace.getConfiguration('projectXray').get<boolean>('analyzeOnSave',true))return;
    if(!doc.fileName.endsWith('.java'))return;
